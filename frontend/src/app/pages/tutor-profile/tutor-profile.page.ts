...
import { 
  personOutline, cameraOutline, star, briefcaseOutline, 
  documentTextOutline, globeOutline, locationOutline, 
  calendarOutline, saveOutline, libraryOutline, 
  closeCircle, checkmarkDoneOutline, searchOutline, folderOpenOutline, 
  createOutline, cashOutline, imageOutline, cloudUploadOutline, addCircleOutline, documentOutline
} from 'ionicons/icons';
...
export class TutorProfilePage implements OnInit {
  ...
  materieDisponibili = ['Matematica', 'Fisica', 'Analisi Matematica 1', 'Informatica', 'Chimica', 'Geometria'];
  materieFiltrate: string[] = [];
  mostraListaMaterie = false;

  lingueDisponibili = ['Italiano', 'Inglese', 'Spagnolo', 'Francese', 'Tedesco'];
  ...
  aggiungiDispensa() {
    if (!this.nuovaDispensa.titolo || this.nuovaDispensa.prezzo === null) return;
